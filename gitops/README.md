...
| `platform/grafana-cloud.sealed.yaml` | `monitoring` / `grafana-cloud` | Grafana Cloud → the stack's details page for the numeric users, and an access policy token with `metrics:write` and `logs:write` — see [platform/README](platform/README.md#telemetry) |
